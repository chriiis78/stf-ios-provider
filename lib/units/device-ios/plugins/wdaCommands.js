...
.define(function(options, vncControl){
    ...
    plugin.openUrlInSafari = async function(url) {
        var safariSessionId = await plugin.launchAppReturnSession("com.apple.mobilesafari")
        log.info('----launch Safari DONE %s', safariSessionId)
        ...
        while (elementId == null && count < 10) {
            ...
            elementId = await getElement(safariSessionId, "name=URL")
            count++
        }
        ...
        while (fieldElementId == null && count < 10) {
            ...
            fieldElementId = await getElement(safariSessionId, "name=URL")
            count++
        }
        log.info('----getElement SearchBar DONE %s', elementId)
    ...
    plugin.openSettingsWifi = async function() {
        var settingsSessionId = await plugin.launchAppReturnSession("com.apple.Preferences")
        log.info('----launch Settings DONE %s', settingsSessionId)
        ...
        while (elementId == null && count < 10) {
            ...
            await new Promise(r => setTimeout(r, 100));
            elementId = await getElement(settingsSessionId, "name=Wi-Fi")
            count++
        }
