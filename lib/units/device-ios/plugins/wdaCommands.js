...
.define(function(options, vncControl){
    ...
    plugin.launchSafari = async function(){
        var body = {
            capabilities:{
                alwaysMatch:{
    ...
    async function getSearchBarElement(sessionId) {
        var body = {
    ...
    async function clickOnSearchBarButton(sessionId, elementId) {
        var body = ""
    ...
    async function sendKeyToSearchBarField(sessionId, elementId, url) {
        var body = {
            value: [url+"\n"]
        }
    ...
    plugin.openUrlInSafari = async function(url) {
        var safariSessionId = await plugin.launchSafari()
        log.info('----launchSafari DONE %s', safariSessionId)
        ...
        while (elementId == null && count < 10) {
            ...
            elementId = await getSearchBarElement(safariSessionId)
            count++
        }
        log.info('----getSearchBarElement DONE %s', elementId)
        ...
        await new Promise(r => setTimeout(r, 100));
        await clickOnSearchBarButton(safariSessionId, elementId)
        log.info('----clickOnSearchBarButton DONE')

        count = 0
        var fieldElementId = null
        while (fieldElementId == null && count < 10) {
            await new Promise(r => setTimeout(r, 500));
            fieldElementId = await getSearchBarElement(safariSessionId)
            count++
        }
        log.info('----getSearchBarElement DONE %s', elementId)
        ...
        await new Promise(r => setTimeout(r, 100));
        await sendKeyToSearchBarField(safariSessionId, fieldElementId, url)
        log.info('----sendKeyToSearchBarField DONE')
    }
